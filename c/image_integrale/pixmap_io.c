...
static int get_pgm_header(int fd, char *magic, int *width, int *height, int* lumin)
{
  char buf[80];
...
static int open_read_pixmap(char *filename, char *magic, int *width, int *height, int* lumin)
{
  int fd;

  if( (fd = open_read(filename)) < 0)
    return fd;
  if( !get_pgm_header(fd, magic, width, height, lumin) )
    {
...
unsigned char *load_pixmap(char *filename, int *width, int *height, int* lumin)
{
  int fd;
  long size;
  unsigned char *data;

  if( (fd = open_read_pixmap(filename, MAGIC_PGM, width, height,lumin)) < 0)
    return NULL;
...
static void put_pgm_header(int fd, char *magic, int width, int height,int lumin, char *filename)
{
  char buf[80];
